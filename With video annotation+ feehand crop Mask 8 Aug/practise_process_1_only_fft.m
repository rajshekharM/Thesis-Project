...
%For making a annotted video by writing annotted frames
anootated_time_cropped_video='time_cropped_video_anotated.avi';
vidObj_annotated = VideoWriter(anootated_time_cropped_video);
open(vidObj_annotated);
vidObj_time_cropped = VideoReader('time_cropped_video.avi');   %read the video frames
%--------------------
...
while(i <= total_windows)
 ...
 %extract the respective frame from video---------------------------
     if(i>1)
        previous_frame_no=t_from_segments(i-1)*fps;    
        next_frame_no=t_from_segments(i)*fps;   
               
     %Write heart-beat on video frame
     
     position = [50 50];
     text_str = ['Heart Rate(current segment): ' num2str(fft_bpm(i),'%d') 'BPM'];
     for current_frame_no=previous_frame_no:next_frame_no
        frame = read(vidObj_time_cropped, current_frame_no);
        annotated_frame = insertText(frame,position,text_str,'FontSize',18,'white','AnchorPoint','LeftBottom');
        writeVideo(vidObj_annotated,annotated_frame);
